---
summary: "Symptom first troubleshooting hub for RedForge"
read_when:
  - RedForge is not working and you need the fastest path to a fix
...
- `RedForge status` → shows configured channels and no obvious auth errors.
- `RedForge status --all` → full report is present and shareable.
- `RedForge gateway probe` → expected gateway target is reachable (`Reachable: yes`). `RPC: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `RedForge gateway status` → `Runtime: running` and `RPC probe: ok`.
- `RedForge doctor` → no blocking config/service errors.
- `RedForge channels status --probe` → reachable gateway returns live per-account
...
- `RedForge logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in RedForge

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `RedForge infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on RedForge agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger RedForge prompts crash the
...
## Plugin install fails with missing RedForge extensions

If install fails with `package.json missing RedForge.extensions`, the plugin package
is using an old shape that RedForge no longer accepts.

Fix in the plugin package:

1. Add `RedForge.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `RedForge plugins install <package>` again.

Example:

```json
{
  "name": "@RedForge/my-plugin",
  "version": "1.2.3",
  "RedForge": {
...
```mermaid
flowchart TD
  A[RedForge is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `RedForge gateway status`
    - `RPC probe: ok`
  ...
  <Accordion title="Cron or heartbeat did not fire or did not deliver">
    ...
    <Accordion title="Exec suddenly asks for approval">
      ```bash
      RedForge config get tools.exec.host
      RedForge config get tools.exec.security
      RedForge config get tools.exec.ask
    ...
    <Accordion title="Browser tool fails">
      ...
      - `RedForge` starts, or `user` can see local Chrome tabs.
      ...
      - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `RedForge browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
