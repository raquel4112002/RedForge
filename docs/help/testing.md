...
RedForge has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Linux VM-backed QA lane: `pnpm RedForge qa suite --runner multipass --scenario channel-chat-baseline`
...
- `pnpm RedForge qa suite --runner multipass`
...
- `pnpm RedForge qa matrix`
  ...
  - Uses the pinned stable Tuwunel image `ghcr.io/matrix-construct/tuwunel:v1.5.1` by default. Override with `RedForge_QA_MATRIX_TUWUNEL_IMAGE` when you need to test a different image.
...
- `pnpm RedForge qa telegram`
  ...
  - Requires `RedForge_QA_TELEGRAM_GROUP_ID`, `RedForge_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `RedForge_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
...
- Projects note:
  ...
  - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-RedForge-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
...
- Pool note:
  ...
  - The shared `scripts/run-vitest.mjs` launcher now also adds `--no-maglev` for Vitest child Node processes by default to reduce V8 compile churn during big local runs. Set `RedForge_VITEST_ENABLE_MAGLEV=1` if you need to compare against stock V8 behavior.
- Fast-local iteration note:
  ...
  - The config keeps `RedForge_VITEST_FS_MODULE_CACHE` enabled on supported hosts; set `RedForge_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want one explicit cache location for direct profiling.
- Perf-debug note:
...
- Useful overrides:
  - `RedForge_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `RedForge_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises RedForge's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `RedForge_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `RedForge_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `RedForge_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.RedForge`.
- Set `RedForge_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `RedForge_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `RedForge_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `RedForge_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `RedForge_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Optional target overrides:
  - `RedForge_ANDROID_NODE_ID` or `RedForge_ANDROID_NODE_NAME`.
  - `RedForge_ANDROID_GATEWAY_URL` / `RedForge_ANDROID_GATEWAY_TOKEN` / `RedForge_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `RedForge_LIVE_TEST=1` if invoking Vitest directly)
- Set `RedForge_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `RedForge_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.7, Grok 4)
  - `RedForge_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `RedForge_LIVE_MODELS="openai/gpt-5.4,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Modern/all sweeps default to a curated high-signal cap; set `RedForge_LIVE_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers:
  - `RedForge_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `RedForge_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates “provider API is broken / key is invalid” from “gateway agent pipeline is broken”
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@RedForge" actually does)
...
- How to select models:
  ...
  - `RedForge_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `RedForge_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
  - Modern/all gateway sweeps default to a curated high-signal cap; set `RedForge_LIVE_GATEWAY_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers (avoid “OpenRouter everything”):
  - `RedForge_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Overrides (optional):
  - `RedForge_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.4"`
  - `RedForge_LIVE_CLI_BACKEND_COMMAND="/full/path/to/codex"`
  - `RedForge_LIVE_CLI_BACKEND_ARGS='["exec","--json","--color","never","--sandbox","read-only","--skip-git-repo-check"]'`
  - `RedForge_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
  - `RedForge_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `RedForge_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `RedForge_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
  - `RedForge_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=0` to disable the default Claude Sonnet -> Opus same-session continuity probe (set to `1` to force it on when the selected model supports a switch target).
...
- It resolves CLI smoke metadata from the owning extension, then installs the matching Linux CLI package (`@anthropic-ai/claude-code`, `@openai/codex`, or `@google/gemini-cli`) into a cached writable prefix at `RedForge_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/RedForge/docker-cli-tools`).
...
- Defaults:
  ...
  - ACP backend: `acpx`
- Overrides:
  - `RedForge_LIVE_ACP_BIND_AGENT=claude`
  - `RedForge_LIVE_ACP_BIND_AGENT=codex`
  - `RedForge_LIVE_ACP_BIND_AGENT=gemini`
  - `RedForge_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `RedForge_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `RedForge_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
...
- Use `RedForge_LIVE_ACP_BIND_AGENTS=claude`, `RedForge_LIVE_ACP_BIND_AGENTS=codex`, or `RedForge_LIVE_ACP_BIND_AGENTS=gemini` to narrow the matrix.
...
- Inside Docker, the runner sets `RedForge_LIVE_ACP_BIND_ACPX_COMMAND=$HOME/.npm-global/bin/acpx` so acpx keeps provider env vars from the sourced profile available to the child harness CLI.
...
- Goal: validate the plugin-owned Codex harness through the normal gateway
  ...
  - select `RedForge_AGENT_RUNTIME=codex`
  - send a first gateway agent turn to `codex/gpt-5.4`
  - send a second turn to the same RedForge session and verify the app-server
    thread can resume
  - run `/codex status` and `/codex models` through the same gateway command
    path
- Test: `src/gateway/gateway-codex-harness.live.test.ts`
- Enable: `RedForge_LIVE_CODEX_HARNESS=1`
- Default model: `codex/gpt-5.4`
- Optional image probe: `RedForge_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- Optional MCP/tool probe: `RedForge_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- The smoke sets `RedForge_AGENT_HARNESS_FALLBACK=none` so a broken Codex
...
```bash
source ~/.profile
RedForge_LIVE_CODEX_HARNESS=1 \
  RedForge_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  RedForge_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  RedForge_LIVE_CODEX_HARNESS_MODEL=codex/gpt-5.4 \
...
- Docker enables the image and MCP/tool probes by default. Set
  `RedForge_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  `RedForge_LIVE_CODEX_HARNESS_MCP_PROBE=0` when you need a narrower debug run.
- Docker also exports `RedForge_AGENT_HARNESS_FALLBACK=none`, matching the live
...
- Single model, direct (no gateway):
  - `RedForge_LIVE_MODELS="openai/gpt-5.4" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `RedForge_LIVE_GATEWAY_MODELS="openai/gpt-5.4" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `RedForge_LIVE_GATEWAY_MODELS="openai/gpt-5.4,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `RedForge_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `RedForge_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API vs Gemini CLI:
  - API: RedForge calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: RedForge shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
`RedForge_LIVE_GATEWAY_MODELS="openai/gpt-5.4,openai-codex/gpt-5.4,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
Include at least one image-capable model in `RedForge_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.

### Aggregators / alternate gateways

If you have keys enabled, we also support testing via:

- OpenRouter: `openrouter/...` (hundreds of models; use `RedForge models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `RedForge models list` / model selection.

- Per-agent auth profiles: `~/.RedForge/agents/<agentId>/agent/auth-profiles.json` (this is what “profile keys” means in the live tests)
- Config: `~/.RedForge/RedForge.json` (or `RedForge_CONFIG_PATH`)
- Legacy state dir: `~/.RedForge/credentials/` (copied into the staged live home when present, but not the main profile-key store)
...
- Enable: `RedForge_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `google`
- Optional narrowing:
  - `RedForge_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google"`
  - `RedForge_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-1,google/gemini-3.1-flash-image-preview"`
  - `RedForge_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit"`
- Optional auth behavior:
  - `RedForge_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides

## Music generation live

- Test: `extensions/music-generation-providers.live.test.ts`
- Enable: `RedForge_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `RedForge_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `RedForge_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.5+"`
- Optional auth behavior:
  - `RedForge_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `RedForge_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
...
- Scope:
  ...
  - Provider-specific Vydra coverage:
    - `RedForge_LIVE_TEST=1 RedForge_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
...
- Optional narrowing:
  - `RedForge_LIVE_VIDEO_GENERATION_PROVIDERS="google,openai,runway"`
  - `RedForge_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
- Optional auth behavior:
  - `RedForge_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Docker live runners default to a smaller smoke cap so a full Docker sweep stays practical:
  `test:docker:live-models` defaults to `RedForge_LIVE_MAX_MODELS=12`, and
  `test:docker:live-gateway` defaults to `RedForge_LIVE_GATEWAY_SMOKE=1`,
  `RedForge_LIVE_GATEWAY_MAX_MODELS=8`,
  `RedForge_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `RedForge_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Override those env vars when you
...
`.pnpm-store`, `.worktrees`, `__RedForge_vitest__`, and app-local `.build` or
Gradle output directories so Docker live runs do not spend minutes copying
machine-specific artifacts.
They also set `RedForge_SKIP_CHANNELS=1` so gateway live probes do not start
...
`RedForge_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
RedForge gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `RedForge/default`, then sends a
...
This lane expects a usable live model key, and `RedForge_PROFILE_FILE`
...
container, starts a second container that spawns `RedForge mcp serve`, then
...
- `RedForge_CONFIG_DIR=...` (default: `~/.RedForge`) mounted to `/home/node/.RedForge`
- `RedForge_WORKSPACE_DIR=...` (default: `~/.RedForge/workspace`) mounted to `/home/node/.RedForge/workspace`
- `RedForge_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- `RedForge_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/RedForge/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `RedForge_LIVE_PROVIDERS` / `RedForge_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `RedForge_DOCKER_AUTH_DIRS=all`, `RedForge_DOCKER_AUTH_DIRS=none`, or a comma list like `RedForge_DOCKER_AUTH_DIRS=.claude,.codex`
- `RedForge_LIVE_GATEWAY_MODELS=...` / `RedForge_LIVE_MODELS=...` to narrow the run
- `RedForge_LIVE_GATEWAY_PROVIDERS=...` / `RedForge_LIVE_PROVIDERS=...` to filter providers in-container
- `RedForge_SKIP_DOCKER_BUILD=1` to reuse an existing `RedForge:local-live` image for reruns that do not need a rebuild
- `RedForge_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `RedForge_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `RedForge_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
