...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in RedForge
...
Running RedForge via Node with `tsx` fails at startup with:

```
[RedForge] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `RedForge status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch RedForge.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node RedForge.mjs status` works on Node 25.
