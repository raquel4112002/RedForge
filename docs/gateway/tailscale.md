...
RedForge can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. RedForge requires a shared password.
...
- `token` (default when `RedForge_GATEWAY_TOKEN` is set)
- `password` (shared secret via `RedForge_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. RedForge verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
RedForge only treats a request as Serve when it arrives from loopback with
...
Prefer `RedForge_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
RedForge gateway --tailscale serve
RedForge gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want RedForge to undo `tailscale serve`
