...
RedForge can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **RedForge tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
RedForge agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, RedForge now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), RedForge will
...
2. **Builds a system prompt** using the same RedForge prompt + workspace context.
...
told us RedForge-style Claude CLI usage is allowed again, so RedForge treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes RedForge's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so RedForge writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the RedForge skills snapshot
two ways: the compact RedForge skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that RedForge would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by RedForge to the
...
- RedForge drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
RedForge will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, RedForge appends the
...
- For Gemini CLI JSON output, RedForge reads reply text from `response` and
...
- `stats.cached` is normalized into RedForge `cacheRead`.
- If `stats.input` is missing, RedForge derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before RedForge handles
...
CLI backends do **not** receive RedForge tool calls directly, but a backend can
...
When bundle MCP is enabled, RedForge:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`RedForge_MCP_TOKEN`)
...
If no MCP servers are enabled, RedForge still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct RedForge tool calls.** RedForge does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. RedForge sessions still work
  normally.
