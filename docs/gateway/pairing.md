...
```bash
RedForge nodes pending
RedForge nodes approve <requestId>
RedForge nodes reject <requestId>
RedForge nodes status
RedForge nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.RedForge`):

- `~/.RedForge/nodes/paired.json`
- `~/.RedForge/nodes/pending.json`

If you override `RedForge_STATE_DIR`, the `nodes/` folder moves with it.
