...
- Default rolling log file is under `/tmp/RedForge/` (one file per day): `RedForge-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.RedForge/RedForge.json`:
  - `logging.file`
  - `logging.level`
...
`RedForge gateway` supports a per-gateway style switch:
