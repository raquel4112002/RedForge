...
read_when:
  - Setting up RedForge for the first time
...
RedForge reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.RedForge/RedForge.json`.

If the file is missing, RedForge uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `RedForge onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    RedForge onboard       # full onboarding flow
    RedForge configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    RedForge config get agents.defaults.workspace
    RedForge config set agents.defaults.heartbeat.every "2h"
    RedForge config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.RedForge/RedForge.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
RedForge only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

Schema tooling notes:

- `RedForge config schema` prints the same JSON Schema family used by Control UI
  and config validation.
- Treat that schema output as the canonical machine-readable contract for
  `RedForge.json`; this overview and the configuration reference summarize it.
...
- Only diagnostic commands work (`RedForge doctor`, `RedForge logs`, `RedForge health`, `RedForge status`)
- Run `RedForge doctor` to see exact issues
- Run `RedForge doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `RedForge.json`.
    ...
    ```bash
    RedForge config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `RedForge_APNS_RELAY_BASE_URL` and `RedForge_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `RedForge_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-RedForge-token`); query-string tokens are rejected.
...
The Gateway watches `~/.RedForge/RedForge.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `RedForge config set` for single keys.
    </Warning>
    ...
    ```bash
    RedForge gateway call config.get --params '{}'  # capture payload.hash
    RedForge gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.RedForge/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    RedForge gateway call config.patch --params '{
...
RedForge reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.RedForge/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, RedForge runs your login shell and imports only the missing keys:
...
Env var equivalent: `RedForge_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${RedForge_GATEWAY_TOKEN}" } },
