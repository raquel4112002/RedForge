...
RedForge’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`RedForge agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-RedForge-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-RedForge-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "RedForge"`, `model: "RedForge/default"`, `model: "RedForge/<agentId>"`, or `x-RedForge-agent-id`
- use `x-RedForge-model` when you want to override the selected agent's backend model
- use `x-RedForge-session-key` for explicit session routing
- use `x-RedForge-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: RedForge reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
RedForge normalizes common OpenAI-style aliases before those counters reach
