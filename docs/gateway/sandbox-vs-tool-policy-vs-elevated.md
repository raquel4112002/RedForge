...
RedForge has three related (but different) controls:
...
Use the inspector to see what RedForge is _actually_ doing:
...
- RedForge validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:RedForge`: all built-in RedForge tools (excludes provider plugins)
