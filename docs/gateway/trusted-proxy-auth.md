...
read_when:
  - Running RedForge behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of RedForge
...
- You run RedForge behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. RedForge checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. RedForge extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that RedForge can verify.
...
- RedForge can stay on loopback HTTP behind the proxy.
...
If RedForge itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://RedForge.example.com
    to: http://RedForge-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
location / {
    ...
    proxy_pass http://RedForge:18789;
...
RedForge rejects ambiguous configurations where both a `gateway.auth.token` (or `RedForge_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-RedForge-scopes`.

Examples:

- `x-RedForge-scopes: operator.read`
- `x-RedForge-scopes: operator.read,operator.write`
- `x-RedForge-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, RedForge honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-RedForge-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-RedForge-scopes` explicitly when you want a trusted-proxy request to
...
`RedForge security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
RedForge rejected a loopback-source trusted-proxy request.
...
3. Update RedForge config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `RedForge security audit` and review findings
