...
RedForge supports OAuth and API keys for model providers. For always-on gateway
...
setup, but RedForge also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `RedForge gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.RedForge/.env` so the daemon can read it:

```bash
cat >> ~/.RedForge/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `RedForge onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.RedForge/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in RedForge as a supported token
path. Anthropic staff has since told us that RedForge-style Claude CLI usage is
...
```bash
RedForge models auth paste-token --provider openrouter
```
...
- If auth exists but RedForge cannot resolve a probeable model candidate for
...
- Anthropic staff told us this RedForge integration path is allowed again.
- RedForge therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `RedForge_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- RedForge retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `RedForge models status --probe` shows omitted
...
Run `RedForge models status` to confirm which profile is expiring. If an
