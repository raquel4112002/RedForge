---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for RedForge agents"
read_when:
...
OpenShell is a managed sandbox backend for RedForge. Instead of running Docker
containers locally, RedForge delegates sandbox lifecycle to the `openshell` CLI,
...
- RedForge Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, RedForge creates an OpenShell
...
- You edit files locally outside RedForge and want those changes visible in the
...
- When the sandbox is first created, RedForge seeds the remote workspace from
...
- RedForge does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside RedForge after the initial seed,
the remote sandbox does **not** see those changes. Use
`RedForge sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"RedForge"`  | Sandbox source for first-time create                  |
...
1. RedForge calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. RedForge calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `RedForge sandbox` commands
