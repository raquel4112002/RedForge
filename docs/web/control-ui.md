...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/RedForge`)
...
If the page fails to load, start the Gateway first: `RedForge gateway`.
...
created. Re-run `RedForge devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `RedForge devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.RedForge/RedForge.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop RedForge`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. RedForge
...
```bash
RedForge gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
RedForge **blocks** Control UI connections without device identity.
...
```bash
RedForge_CONTROL_UI_BASE_PATH=/RedForge/ pnpm ui:build
```
