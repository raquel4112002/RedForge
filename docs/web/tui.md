...
```bash
RedForge tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `RedForge tui --deliver`
...
- Local shell commands receive `RedForge_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `RedForge_THEME=light` before launching `RedForge tui`.
- To force the original dark palette instead, set `RedForge_THEME=dark`.
...
- Check the Gateway logs: `RedForge logs --follow`.
- Confirm the agent can run: `RedForge status` and `RedForge models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `RedForge agents list` and your routing config.
