...
CLI onboarding is the **recommended** way to set up RedForge on macOS,
...
`RedForge dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`RedForge configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.RedForge/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `RedForge doctor` first.
</Note>
...
Use `RedForge agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.RedForge/workspace-<agentId>`.
...
- CLI command reference: [`RedForge onboard`](/cli/onboard)
