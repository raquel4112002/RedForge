...
```bash
RedForge nodes describe --node <idOrNameOrIp>
RedForge nodes canvas snapshot --node <idOrNameOrIp>
RedForge logs --follow
```
...
RedForge nodes status
RedForge approvals get --node <idOrNameOrIp>
RedForge approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`RedForge approvals get --node ...`), not in the gateway pairing record.
