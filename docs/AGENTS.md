...
- Docs are hosted on Mintlify (`https://docs.RedForge.ai`).
...
- Foreign-language docs are not maintained in this repo. The generated publish output lives in the separate `RedForge/docs` repo (often cloned locally as `../RedForge-docs`).
...
- Pipeline: update English docs here, update `docs/.i18n/glossary.<locale>.json` as needed, then let the publish-repo sync and `scripts/docs-i18n` run in `RedForge/docs`.
