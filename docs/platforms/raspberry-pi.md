---
summary: "RedForge on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up RedForge on a Raspberry Pi
  - Running RedForge on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# RedForge on Raspberry Pi

## Goal

Run a persistent, always-on RedForge Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u RedForge-gateway.service -f
```

## 9) Access the RedForge Dashboard
...
```bash
ssh user@gateway-host 'RedForge dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/RedForge-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/RedForge-compile-cache
mkdir -p /var/tmp/RedForge-compile-cache
export RedForge_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `RedForge_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running RedForge, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit RedForge-gateway.service
```

```ini
[Service]
Environment=RedForge_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/RedForge-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep RedForge state/cache on SSD-backed storage to avoid SD-card
...
Most RedForge features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled RedForge-gateway.service

# Enable if not
systemctl --user enable RedForge-gateway.service

# Start on boot
systemctl --user start RedForge-gateway.service
```
...
```bash
# Check logs
journalctl --user -u RedForge-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/RedForge  # if using hackable install
npm run build
systemctl --user restart RedForge-gateway.service
```
