...
- Same path: run the app from a fixed location (for RedForge, `dist/RedForge.app`).
...
```bash
sudo tccutil reset Accessibility ai.RedForge.mac
sudo tccutil reset ScreenCapture ai.RedForge.mac
...
Workaround: move files into the RedForge workspace (`~/.RedForge/workspace`) if you want to avoid per-folder grants.
