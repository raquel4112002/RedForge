...
service via the external `RedForge` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.RedForge.gateway`
  (or `ai.RedForge.<profile>` when using `--profile`/`RedForge_PROFILE`; legacy `com.RedForge.*` is supported).
