---
summary: "RedForge on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up RedForge on Oracle Cloud
  - Looking for low-cost VPS hosting for RedForge
  - Want 24/7 RedForge on a small server
title: "Oracle Cloud (Platform)"
---

# RedForge on Oracle Cloud (OCI)

## Goal

Run a persistent RedForge Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for RedForge (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname RedForge
...
sudo tailscale up --ssh --hostname=RedForge
```

This enables Tailscale SSH, so you can connect via `ssh RedForge` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@RedForge` (or use the Tailscale IP).
...
RedForge config set gateway.bind loopback

# Require auth for the Gateway + Control UI
RedForge config set gateway.auth.mode token
RedForge doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
RedForge config set gateway.tailscale.mode serve
RedForge config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart RedForge-gateway.service
```
...
# Check daemon status
systemctl --user status RedForge-gateway.service
...
```
https://RedForge.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `RedForge security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.RedForge`
- **Security audit:** `RedForge security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@RedForge
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=RedForge --reset
```
...
journalctl --user -u RedForge-gateway.service -n 50
```
...
- `~/.RedForge/` — `RedForge.json`, per-agent `auth-profiles.json`, channel/provider state, and session data
- `~/.RedForge/workspace/` — workspace (SOUL.md, memory, artifacts)
