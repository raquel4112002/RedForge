---
summary: "RedForge on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up RedForge on DigitalOcean
  - Looking for cheap VPS hosting for RedForge
title: "DigitalOcean (Platform)"
---

# RedForge on DigitalOcean

## Goal

Run a persistent RedForge Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
RedForge config set gateway.bind tailnet
RedForge gateway restart
```
...
```bash
RedForge channels login whatsapp
# Scan QR code
```
...
- `~/.RedForge/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u RedForge-gateway.service --no-pager -n 50
```
