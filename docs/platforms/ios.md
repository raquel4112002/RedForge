...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `RedForge.internal.`), **or**
...
Run `RedForge devices list` again before approval.
...
- `RedForge_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine RedForge iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight RedForge builds.
...
```bash
export RedForge_APNS_TEAM_ID="TEAMID"
export RedForge_APNS_KEY_ID="KEYID"
export RedForge_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.RedForge/credentials/apns
chmod 700 ~/.RedForge/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.RedForge/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.RedForge/credentials/apns/AuthKey_KEYID.p8
export RedForge_APNS_PRIVATE_KEY_PATH="$HOME/.RedForge/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_RedForge-gw._tcp` on `local.` and, when configured, the same
...
`RedForge.internal.`) and Tailscale split DNS.
...
```bash
RedForge nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__RedForge__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__RedForge__/canvas/` and `/__RedForge__/a2ui/`.
...
```bash
RedForge nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__RedForge; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
RedForge nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `RedForge devices list` and approve manually.
