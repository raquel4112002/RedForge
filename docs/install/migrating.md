---
summary: "Move (migrate) an RedForge install from one machine to another"
read_when:
  - You are moving RedForge to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating RedForge to a New Machine

This guide moves an RedForge gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.RedForge/` by default) and your **workspace**, you preserve:

- **Config** -- `RedForge.json` and all gateway settings
...
<Tip>
Run `RedForge status` on the old machine to confirm your state directory path.
Custom profiles use `~/.RedForge-<profile>/` or a path set via `RedForge_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf RedForge-state.tgz .RedForge
    ```

    If you use multiple profiles (e.g. `~/.RedForge-work`), archive each separately.

  </Step>

  <Step title="Install RedForge on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.RedForge/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `RedForge_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `RedForge doctor`.
  </Accordion>

  <Accordion title="Copying only RedForge.json">
...
- [ ] `RedForge status` shows the gateway running
