---
summary: "Deploy RedForge Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run RedForge on a Kubernetes cluster
  - You want to test RedForge in a Kubernetes environment
title: "Kubernetes"
---

# RedForge on Kubernetes

A minimal starting point for running RedForge on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

RedForge is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/RedForge 18789:18789 -n RedForge
...
```bash
kubectl get secret RedForge-secrets -n RedForge -o jsonpath='{.data.RedForge_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: RedForge (configurable via RedForge_NAMESPACE)
├── Deployment/RedForge        # Single pod, init container + gateway
├── Service/RedForge           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/RedForge-config  # RedForge.json + AGENTS.md
└── Secret/RedForge-secrets    # Gateway token + API keys
```
...
Edit `RedForge.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret RedForge-secrets -n RedForge \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/RedForge -n RedForge
```

### Custom namespace

```bash
RedForge_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/RedForge/RedForge:latest # or pin to a specific version from https://github.com/RedForge/RedForge/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # RedForge.json + AGENTS.md
