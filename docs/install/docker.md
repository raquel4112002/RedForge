---
summary: "Optional Docker-based setup and onboarding for RedForge"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run RedForge on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export RedForge_IMAGE="ghcr.io/RedForge/RedForge:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/RedForge/RedForge/pkgs/container/RedForge).
  ...
  <Step title="Complete onboarding">
    ...
    `RedForge-gateway` directly. `RedForge-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm RedForge-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm RedForge-cli channels login

    # Telegram
    docker compose run --rm RedForge-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm RedForge-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t RedForge:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node RedForge-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `RedForge_EXTRA_MOUNTS`
or `RedForge_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `RedForge-cli` shares `RedForge-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d RedForge-gateway`, run onboarding
and setup-time config writes through `RedForge-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `RedForge_IMAGE`               | Use a remote image instead of building locally                   |
| `RedForge_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `RedForge_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `RedForge_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `RedForge_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `RedForge_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `RedForge_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
```bash
docker compose exec RedForge-gateway node dist/index.js health --token "$RedForge_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `RedForge_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `RedForge_CONFIG_DIR` to `/home/node/.RedForge` and
`RedForge_WORKSPACE_DIR` to `/home/node/.RedForge/workspace`, so those paths
survive container replacement.

That mounted config directory is where RedForge keeps:

- `RedForge.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `RedForge_GATEWAY_TOKEN`

For full persistence details on VM deployments, see
[Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** watch `media/`, session JSONL files, `cron/runs/*.jsonl`,
and rolling file logs under `/tmp/RedForge/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/RedForge/RedForge/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export RedForge_SANDBOX=1
    export RedForge_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm RedForge-cli gateway probe
    docker compose run -T --rm RedForge-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `RedForge-cli` uses `network_mode: "service:RedForge-gateway"` so CLI
    ...
    `no-new-privileges` on `RedForge-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.RedForge`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/RedForge-config /path/to/RedForge-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export RedForge_HOME_VOLUME="RedForge_home"`
    2. **Bake system deps**: `export RedForge_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `RedForge_HOME_VOLUME` or `RedForge_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/RedForge/RedForge/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    RedForge runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm RedForge-cli dashboard --no-open
    docker compose run --rm RedForge-cli devices list
    docker compose run --rm RedForge-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm RedForge-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm RedForge-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping RedForge up to date
