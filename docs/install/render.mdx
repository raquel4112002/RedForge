---
summary: "Deploy RedForge on Render with Infrastructure-as-Code"
read_when:
  - Deploying RedForge to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy RedForge on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/RedForge/RedForge)
...
repository configures everything needed to run RedForge:
...
services:
  - type: web
    ...
    envVars:
      - key: RedForge_GATEWAY_PORT
        value: "8080"
      - key: RedForge_STATE_DIR
        value: /data/.RedForge
      - key: RedForge_WORKSPACE_DIR
        value: /data/workspace
      - key: RedForge_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means RedForge state
...
`RedForge_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original RedForge repository, Render will not auto-deploy your RedForge. To update it, run a manual Blueprint sync from the dashboard.
...
For RedForge, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with RedForge state plus any configured
...
- Missing `RedForge_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `RedForge_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `RedForge backup create` in the Render shell.
