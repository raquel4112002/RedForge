---
summary: "ClawDock shell helpers for Docker-based RedForge installs"
read_when:
  - You run RedForge with Docker often and want shorter day-to-day commands
...
ClawDock is a small shell-helper layer for Docker-based RedForge installs.
...
| `clawdock-cli <command>`  | Run RedForge CLI commands in Docker           |
...
| `clawdock-cd`          | Jump to the RedForge project directory  |
| `clawdock-config`      | Open `~/.RedForge`                      |
...
- `~/.RedForge/.env` for env-backed provider keys and bot tokens
- `~/.RedForge/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.RedForge/RedForge.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `RedForge.json` quickly. It redacts `.env` values in its printed output.
