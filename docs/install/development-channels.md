...
RedForge ships three update channels:
...
```bash
RedForge update --channel stable
...
- **`dev`**: ensures a git checkout (default `~/RedForge`, override with
  `RedForge_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
RedForge update --tag 2026.4.1-beta.1
...
RedForge update --tag RedForge@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `RedForge update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  RedForge prompts for confirmation (skip with `--yes`).
...
Preview what `RedForge update` would do without making changes:

```bash
RedForge update --dry-run
RedForge update --channel beta --dry-run
RedForge update --tag 2026.4.1-beta.1 --dry-run
RedForge update --dry-run --json
```
...
When you switch channels with `RedForge update`, RedForge also syncs plugin
sources:
