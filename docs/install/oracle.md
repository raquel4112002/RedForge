---
summary: "Host RedForge on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up RedForge on Oracle Cloud
  - Looking for free VPS hosting for RedForge
...
Run a persistent RedForge Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@RedForge`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=RedForge --reset` to re-authenticate.

**Gateway will not start** -- Run `RedForge doctor --non-interactive` and check logs with `journalctl --user -u RedForge-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep RedForge up to date
