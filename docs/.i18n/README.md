...
- repo: `RedForge/docs`
- local checkout: `~/Projects/RedForge-docs`

## Source of truth

- English docs are authored in `RedForge/RedForge`.
...
1. Edit English docs in `RedForge/RedForge`.
2. Push to `main`.
3. `RedForge/RedForge/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `RedForge/docs`.
...
5. `RedForge/docs/.github/workflows/translate-zh-cn.yml` refreshes `docs/zh-CN/**` once a day, on demand, and after source-repo release dispatches.
6. `RedForge/docs/.github/workflows/translate-ja-jp.yml` does the same for `docs/ja-JP/**`.
7. `RedForge/docs/.github/workflows/translate-es.yml`, `translate-pt-br.yml`, `translate-ko.yml`, `translate-de.yml`, `translate-fr.yml`, `translate-ar.yml`, `translate-it.yml`, `translate-tr.yml`, `translate-uk.yml`, `translate-id.yml`, and `translate-pl.yml` do the same for `docs/es/**`, `docs/pt-BR/**`, `docs/ko/**`, `docs/de/**`, `docs/fr/**`, `docs/ar/**`, `docs/it/**`, `docs/tr/**`, `docs/uk/**`, `docs/id/**`, and `docs/pl/**`.
...
- Sync metadata is written to `.RedForge-sync/source.json` in the publish repo.
- Source repo secret: `RedForge_DOCS_SYNC_TOKEN`
- Publish repo secret: `RedForge_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the matching `Translate <locale>` workflow in `RedForge/docs` first.
