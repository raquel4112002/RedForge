...
Tlon is a decentralized messenger built on Urbit. RedForge connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current RedForge releases, so normal packaged
...
```bash
RedForge plugins install @RedForge/tlon
```

Local checkout (when running from a git repo):

```bash
RedForge plugins install ./path/to/local/tlon-plugin
```
...
By default, RedForge blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `RedForge message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, RedForge replies in-thread.
