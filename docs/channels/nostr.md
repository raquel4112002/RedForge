...
read_when:
  - You want RedForge to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables RedForge to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current RedForge releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`RedForge onboard`) and `RedForge channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
RedForge plugins install @RedForge/nostr
```

Use a local checkout (dev workflows):

```bash
RedForge plugins install --link <path-to-local-nostr-plugin>
```
...
RedForge channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "RedForge",
        displayName: "RedForge",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "RedForge@example.com",
        lud16: "RedForge@example.com",
      },
    },
  },
}
