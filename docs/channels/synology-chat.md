---
summary: "Synology Chat webhook setup and RedForge config"
read_when:
  - Setting up Synology Chat with RedForge
...
Synology Chat ships as a bundled plugin in current RedForge releases, so normal
...
```bash
RedForge plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `RedForge onboard` now shows Synology Chat in the same channel setup list as `RedForge channels add`.
   - Non-interactive setup: `RedForge channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your RedForge gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in RedForge.
   - Guided: `RedForge onboard`
   - Direct: `RedForge channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- RedForge accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `RedForge pairing list synology-chat`
  - `RedForge pairing approve synology-chat <CODE>`
...
```bash
RedForge message send --channel synology-chat --target 123456 --text "Hello from RedForge"
RedForge message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. RedForge now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached RedForge
- `Rate limit exceeded`:
