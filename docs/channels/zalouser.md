...
read_when:
  - Setting up Zalo Personal for RedForge
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside RedForge.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current RedForge releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `RedForge plugins install @RedForge/zalouser`
- Or from a source checkout: `RedForge plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `RedForge channels login --channel zalouser`
...
```bash
RedForge directory self --channel zalouser
RedForge directory peers list --channel zalouser --query "name"
RedForge directory groups list --channel zalouser --query "work"
```
...
- `RedForge pairing list zalouser`
- `RedForge pairing approve zalouser <code>`
...
- On startup, RedForge resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, RedForge stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in RedForge state. Example:
...
- RedForge sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, RedForge sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `RedForge channels logout --channel zalouser && RedForge channels login --channel zalouser`
...
- The channel now runs fully in RedForge without external CLI binaries.
