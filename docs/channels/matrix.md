...
Matrix is a bundled channel plugin for RedForge.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Bundled plugin

Matrix ships as a bundled plugin in current RedForge releases, so normal
...
```bash
RedForge plugins install @RedForge/matrix
```

Install from a local checkout:

```bash
RedForge plugins install ./path/to/local/matrix-plugin
```
...
- To resolve room names before saving, use `RedForge channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "RedForge Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.RedForge/credentials/matrix/`.
The default account uses `credentials.json`; named accounts use `credentials-<account>.json`.
When cached credentials exist there, RedForge treats Matrix as configured for setup, doctor, and channel-status discovery even if current auth is not set directly in config.
...
`autoJoin` applies to all Matrix invites, including DM-style invites. RedForge cannot reliably
...
Set `channels.matrix.streaming` to `"partial"` when you want RedForge to send a single live preview
...
- `streaming: "off"` is the default. RedForge waits for the final reply and sends it once.
...
- If the preview no longer fits in one Matrix event, RedForge stops preview streaming and falls back to normal final delivery.
- Media replies still send attachments normally. If a stale preview can no longer be reused safely, RedForge redacts it before sending the final media reply.
...
- bot user = the RedForge Matrix account that sends the reply
...
1. Configure RedForge to use quiet previews:
...
RedForge marks finalized text-only preview edits with:

```json
{
  "com.RedForge.finalized_preview": true
}
```
...
```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/RedForge-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.RedForge\\.finalized_preview",
        "value": true
      },
...
- `RedForge-finalized-preview-botname`: a rule ID unique to this bot for this receiving user
- `@bot:example.org`: your RedForge Matrix bot MXID, not the receiving user's MXID
...
- If one receiving user should notify for multiple RedForge Matrix bot accounts, create one rule per bot with a unique rule ID for each sender match.
- A simple pattern is `RedForge-finalized-preview-<botname>`, such as `RedForge-finalized-preview-ops` or `RedForge-finalized-preview-support`.

The rule is evaluated against the event sender:

- authenticate with the receiving user's token
- match `sender` against the RedForge bot MXID
...
- This only affects text-only preview edits that RedForge can safely finalize in place. Media fallbacks and stale-preview fallbacks still use normal Matrix delivery.
...
- No special `homeserver.yaml` change is required for finalized RedForge preview notifications.
...
By default, Matrix messages from other configured RedForge Matrix accounts are ignored.
...
- RedForge still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; RedForge treats "bot-authored" as "sent by another configured Matrix account on this RedForge gateway".
...
```bash
RedForge matrix verify status --include-recovery-key --json
```
...
```bash
RedForge matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
RedForge matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
RedForge matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
RedForge matrix verify backup restore
```

Verbose restore diagnostics:

```bash
RedForge matrix verify backup restore --verbose
```
...
```bash
RedForge matrix verify status --account assistant
RedForge matrix verify backup restore --account assistant
RedForge matrix devices list --account assistant
```
...
RedForge treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `RedForge matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for RedForge to treat the device as fully verified.

### What bootstrap does

`RedForge matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, RedForge tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
backup baseline for future messages, use `RedForge matrix verify backup reset --yes`.
Do this only when you accept that unrecoverable old encrypted history will stay
unavailable and that RedForge may recreate secret storage if the current backup
...
If startup finds broken bootstrap state and `channels.matrix.password` is configured, RedForge can attempt a stricter repair path.
If the current device is already owner-signed, RedForge preserves that identity instead of resetting it automatically.
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by RedForge.
For self-verification flows, RedForge also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, RedForge auto-accepts the request and then waits for the SAS flow to proceed normally.
...
RedForge does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old RedForge-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
RedForge matrix devices list
```

Remove stale RedForge-managed devices with:

```bash
RedForge matrix devices prune-stale
```
...
`~/.RedForge/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, RedForge reuses the best existing
...
```bash
RedForge matrix profile set --name "RedForge Assistant"
RedForge matrix profile set --avatar-url https://cdn.example.org/avatar.png
```

Add `--account <id>` when you want to target a named Matrix account explicitly.

Matrix accepts `mxc://` avatar URLs directly. When you pass an `http://` or `https://` avatar URL, RedForge uploads it to Matrix first and stores the resolved `mxc://` URL back into `channels.matrix.avatarUrl` (or the selected account override).
...
- Same-session DM user-target reuse only kicks in when the current session metadata proves the same DM peer on the same Matrix account; otherwise RedForge falls back to normal user-scoped routing.
- When RedForge sees a Matrix DM room collide with another DM room on the same shared Matrix DM session, it posts a one-time `m.notice` in that room with the `/focus` escape hatch when thread bindings are enabled and the `dm.sessionScope` hint.
...
- `threadBindings.spawnAcpSessions` is only required for `/acp spawn --thread auto|here`, where RedForge needs to create or bind a child Matrix thread.
...
Ack reactions use the standard RedForge resolution order:
...
- Matrix room history is pending-only: RedForge buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
RedForge pairing list matrix
RedForge pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, RedForge reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state gets out of sync, RedForge can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
RedForge matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
RedForge matrix direct repair --user-id @alice:example.org
```
...
Partial shared auth defaults do not create a separate implicit default account by themselves. RedForge only synthesizes the top-level `default` account when that default has fresh auth (`homeserver` plus `accessToken`, or `homeserver` plus `userId` and `password`); named accounts can still stay discoverable from `homeserver` plus `userId` when cached credentials satisfy auth later.
...
Set `defaultAccount` when you want RedForge to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `RedForge matrix verify ...` and `RedForge matrix devices ...` when you want to override that implicit selection for one command.
...
By default, RedForge blocks private/internal Matrix homeservers for SSRF protection unless you
...
RedForge uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere RedForge asks you for a room or user target:
...
- `allowBots`: allow messages from other configured RedForge Matrix accounts (`true` or `"mentions"`).
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; RedForge does not trust alias state claimed by the invited room.
- `dm`: DM policy block (`enabled`, `policy`, `allowFrom`, `sessionScope`, `threadReplies`).
- `dm.policy`: controls DM access after RedForge has joined the room and classified it as a DM. It does not change whether an invite is auto-joined.
