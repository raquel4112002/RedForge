...
Nextcloud Talk ships as a bundled plugin in current RedForge releases, so
...
```bash
RedForge plugins install @RedForge/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
RedForge plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "RedForge" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `RedForge pairing list nextcloud-talk`
  - `RedForge pairing approve nextcloud-talk <CODE>`
