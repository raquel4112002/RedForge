...
Zalo ships as a bundled plugin in current RedForge releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `RedForge plugins install @RedForge/zalo`
- Or from a source checkout: `RedForge plugins install ./path/to/local/zalo-plugin`
...
This page reflects current RedForge behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `RedForge pairing list zalo`
  - `RedForge pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in RedForge.
...
- Example: `RedForge message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `RedForge channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `RedForge logs --follow`
