...
read_when:
  - Setting up Twitch chat integration for RedForge
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. RedForge connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current RedForge releases, so normal
...
```bash
RedForge plugins install @RedForge/twitch
```

Local checkout (when running from a git repo):

```bash
RedForge plugins install ./path/to/local/twitch-plugin
```
...
5. Configure the token:
   - Env: `RedForge_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "RedForge", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use RedForge_TWITCH_ACCESS_TOKEN env var)
...
```bash
RedForge_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
