...
read_when:
  - You want to connect RedForge to LINE
...
LINE connects to RedForge via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current RedForge releases, so normal
...
```bash
RedForge plugins install @RedForge/line
```

Local checkout (when running from a git repo):

```bash
RedForge plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so RedForge applies strict pre-auth body limits and timeout before verification.
- RedForge processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
RedForge pairing list line
RedForge pairing approve line <CODE>
```
