...
RedForge routes replies **back to the channel where a message came from**. The
...
RedForge infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, RedForge still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when RedForge would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.RedForge/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.RedForge`):

- `~/.RedForge/agents/<agentId>/sessions/sessions.json`
