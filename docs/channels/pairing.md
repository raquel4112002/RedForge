...
read_when:
  ...
  - Reviewing RedForge security posture
title: "Pairing"
---

# Pairing

“Pairing” is RedForge’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `RedForge-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.RedForge/credentials/`:
...
3. On your phone, open the RedForge iOS app → Settings → Gateway.
...
- The legacy `node.pair.*` API (CLI: `RedForge nodes pending|approve|reject|rename`) is a
