---
summary: "Install and use Codex, Claude, and Cursor bundles as RedForge plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how RedForge maps bundle content into native features
...
RedForge can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
RedForge maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native RedForge plugins. Native plugins run
...
of requiring authors to rewrite them as native RedForge plugins, RedForge
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    RedForge plugins install ./my-bundle

    # Archive
    RedForge plugins install ./my-bundle.tgz
...
## What RedForge maps from bundles

Not every bundle feature runs in RedForge today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal RedForge skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | RedForge-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
...
- bundle skill roots load as normal RedForge skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal RedForge skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal RedForge hook-pack
...
- RedForge merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- RedForge exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, RedForge uses `sse`
...
RedForge registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- RedForge sanitizes shell override keys before applying them
...
- RedForge loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `RedForge plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but RedForge does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit RedForge best when they use skill roots and RedForge-style
...
RedForge checks for native plugin format first:

1. `RedForge.plugin.json` or valid `package.json` with `RedForge.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, RedForge uses the native path. This prevents
...
- RedForge does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `RedForge plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. RedForge does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    RedForge hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
