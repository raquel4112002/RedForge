...
summary: "Create your first RedForge plugin in minutes"
read_when:
  - You want to create a new RedForge plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to RedForge
---
...
You do not need to add your plugin to the RedForge repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`RedForge plugins install <package-name>`. RedForge tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect RedForge to a messaging platform (Discord, IRC, etc.)
  </Card>
...
`RedForge/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json RedForge.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to RedForge",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    RedForge plugins install clawhub:@myorg/RedForge-my-plugin
    ```

    RedForge also checks ClawHub before npm for bare package specs like
    `@myorg/RedForge-my-plugin`.
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, RedForge retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `RedForge/plugin-sdk/error-runtime`
...
Always import from focused `RedForge/plugin-sdk/<subpath>` paths:

```typescript
import { definePluginEntry } from "RedForge/plugin-sdk/plugin-entry";
import { createPluginRuntimeStore } from "RedForge/plugin-sdk/runtime-store";

// Wrong: monolithic root (deprecated, will be removed)
import { ... } from "RedForge/plugin-sdk";
```
...
package-root seam instead of promoting it into `RedForge/plugin-sdk/*`.

Some generated `RedForge/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `RedForge` metadata</Check>
<Check>**RedForge.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [RedForge/RedForge](https://github.com/RedForge/RedForge/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official RedForge X account [@RedForge](https://x.com/RedForge) for release announcements.
