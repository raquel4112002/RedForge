...
read_when:
  ...
  - You want a reference for all registration methods on RedForgePluginApi
...
prefer `RedForge/plugin-sdk/channel-core`; keep `RedForge/plugin-sdk/core` for
...
`RedForge/plugin-sdk/slack`, `RedForge/plugin-sdk/discord`,
`RedForge/plugin-sdk/signal`, `RedForge/plugin-sdk/whatsapp`, or
...
| `plugin-sdk/config-schema`  | `RedForgeSchema`                                                                                                                       |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `RedForge.json` Zod schema export (`RedForgeSchema`) |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, RedForge tool bridge helpers, and attempt result utilities |
...
The `register(api)` callback receives an `RedForgePluginApi` object with these
methods:
...
- User config still wins. RedForge merges `agents.defaults.cliBackends.<id>` over the
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `RedForge/plugin-sdk/memory-host-core` instead of reaching into a specific
...
| `api.config`             | `RedForgeConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `RedForge/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when RedForge is already running. If no runtime
...
- `@RedForge/openrouter-provider`: `api.ts` exports the provider builder plus
  onboarding/config helpers

<Warning>
  Extension production code should also avoid `RedForge/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `RedForge/plugin-sdk/speech`, `.../provider-model-shared`, or another
