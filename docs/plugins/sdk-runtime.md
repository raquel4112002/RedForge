...
`runEmbeddedAgent(...)` is the neutral helper for starting a normal RedForge
...
Bind a Task Flow runtime to an existing RedForge session key or trusted tool
...
trusted RedForge session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "RedForge plugin SDK", count: 5 },
});
```
