...
An **agent harness** is the low level executor for one prepared RedForge agent
...
runtime and the normal RedForge provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the RedForge
  session transcript
...
Before a harness is selected, RedForge has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the RedForge transcript/session file
...
**Import:** `RedForge/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "RedForge/plugin-sdk/agent-harness";
...
RedForge chooses a harness after provider/model resolution:

1. `RedForge_AGENT_RUNTIME=<id>` forces a registered harness with that id.
2. `RedForge_AGENT_RUNTIME=pi` forces the built-in PI harness.
3. `RedForge_AGENT_RUNTIME=auto` asks registered harnesses if they support the
   resolved provider/model.
4. If no registered harness matches, RedForge uses PI unless PI fallback is
   disabled.

Forced plugin harness failures surface as run failures. In `auto` mode,
RedForge may fall back to PI when the selected plugin harness fails before a
turn has produced side effects. Set `RedForge_AGENT_HARNESS_FALLBACK=none` or
...
RedForge. The harness then claims that provider in `supports(...)`.
...
- app-server request: RedForge sends the bare model id to Codex and lets the
  harness talk to the native app-server protocol

The Codex plugin is additive. Plain `openai/gpt-*` refs remain OpenAI provider
refs and continue to use the normal RedForge provider path. Select `codex/gpt-*`
...
RedForge requires Codex app-server `0.118.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
RedForge only runs against the protocol surface it has been tested with.

### Native Codex harness mode

The bundled `codex` harness is the native Codex mode for embedded RedForge
agent turns. Enable the bundled `codex` plugin first, and include `codex` in
`plugins.allow` if your config uses a restrictive allowlist. It is different
from `openai-codex/*`:

- `openai-codex/*` uses ChatGPT/Codex OAuth through the normal RedForge provider
  path.
- `codex/*` uses the bundled Codex provider and routes the turn through Codex
  app-server.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. RedForge still owns the chat channel,
...
By default, RedForge runs embedded agents with `agents.defaults.embeddedHarness`
...
plugin harness fails before producing output, RedForge falls back to PI.
...
an explicit `runtime: "pi"` or `RedForge_AGENT_RUNTIME=pi`.
...
want RedForge to silently fall back to PI, keep `runtime: "auto"` and disable
the fallback:
...
`RedForge_AGENT_RUNTIME` still overrides the configured runtime. Use
`RedForge_AGENT_HARNESS_FALLBACK=none` to disable PI fallback from the
environment.

```bash
RedForge_AGENT_RUNTIME=codex \
RedForge_AGENT_HARNESS_FALLBACK=none \
...
Keep that binding explicitly associated with the RedForge session, and keep
mirroring user-visible assistant/tool output into the RedForge transcript.

The RedForge transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so RedForge can
clear it when the owning RedForge session is reset.

## Tool and media results

Core constructs the RedForge tool list and passes it into the prepared attempt.
