...
read_when:
  - You want to use Groq with RedForge
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. RedForge connects
...
Groq's model catalog changes frequently. Run `RedForge models list | grep groq`
...
<Tip>
Use `RedForge models list --provider groq` for the most up-to-date list of
...
media-understanding provider, RedForge uses Groq's `whisper-large-v3-turbo`
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    gateway processes. Use `~/.RedForge/.env` or `env.shellEnv` config for
