---
summary: "Run RedForge with vLLM (OpenAI-compatible local server)"
read_when:
  - You want to run RedForge against a local vLLM server
...
vLLM can serve open-source (and some custom) models via an **OpenAI-compatible** HTTP API. RedForge connects to vLLM using the `openai-completions` API.

RedForge can also **auto-discover** available models from vLLM when you opt in with `VLLM_API_KEY` (any value works if your server does not enforce auth) and you do not define an explicit `models.providers.vllm` entry.
...
<Steps>
  ...
  <Step title="Verify the model is available">
    ```bash
    RedForge models list --provider vllm
    ```
  </Step>
</Steps>
...
When `VLLM_API_KEY` is set (or an auth profile exists) and you **do not** define `models.providers.vllm`, RedForge queries:
...
<AccordionGroup>
  <Accordion title="Proxy-style behavior">
    ...
    | Hidden RedForge attribution headers | Not injected on custom base URLs |
...
<AccordionGroup>
  ...
  <Accordion title="Auth errors on requests">
    ...
    <Tip>
    If your vLLM server does not enforce auth, any non-empty value for `VLLM_API_KEY` works as an opt-in signal for RedForge.
    </Tip>

  </Accordion>

  <Accordion title="No models discovered">
    Auto-discovery requires `VLLM_API_KEY` to be set **and** no explicit `models.providers.vllm` config entry. If you have defined the provider manually, RedForge skips discovery and uses only your declared models.
  </Accordion>
</AccordionGroup>
