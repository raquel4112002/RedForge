---
summary: "Run RedForge with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run RedForge against a local SGLang server
...
RedForge can connect to SGLang using the `openai-completions` API.

RedForge can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, RedForge will query:
