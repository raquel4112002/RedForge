---
summary: "Use Vydra image, video, and speech in RedForge"
read_when:
  - You want Vydra media generation in RedForge
...
RedForge uses the same `VYDRA_API_KEY` for all three capabilities.
...
<Steps>
  <Step title="Run interactive onboarding">
    ```bash
    RedForge onboard --auth-choice vydra-api-key
    ```
...
<AccordionGroup>
  <Accordion title="Image generation">
    ...
    Current bundled support is text-to-image only. Vydra's hosted edit routes expect remote image URLs, and RedForge does not add a Vydra-specific upload bridge in the bundled plugin yet.
  ...
  <Accordion title="Video live tests">
    ...
    ```bash
    RedForge_LIVE_TEST=1 \
    RedForge_LIVE_VYDRA_VIDEO=1 \
    ...
    ```bash
    export RedForge_LIVE_VYDRA_KLING_IMAGE_URL="https://example.com/reference.png"
    ```
