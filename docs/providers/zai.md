---
summary: "Use Z.AI (GLM models) with RedForge"
read_when:
  - You want Z.AI / GLM models in RedForge
...
for authentication. Create your API key in the Z.AI console. RedForge uses the `zai` provider
with a Z.AI API key.
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. RedForge detects the matching Z.AI endpoint from the key and applies the correct base URL automatically.
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        RedForge onboard --auth-choice zai-coding-cn

        # General API
        RedForge onboard --auth-choice zai-global

        # General API CN (China region)
        RedForge onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
RedForge currently seeds the bundled `zai` provider with:
