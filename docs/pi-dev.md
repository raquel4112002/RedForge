...
This guide summarizes a sane workflow for working on the pi integration in RedForge.
...
```bash
RedForge_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm RedForge agent --message "Hello" --thinking low`
...
State lives under the RedForge state directory. Default is `~/.RedForge`. If `RedForge_STATE_DIR` is set, use that directory instead.

To reset everything:

- `RedForge.json` for config
