---
summary: "CLI reference for `RedForge models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
RedForge models set <model-or-alias>
RedForge models scan
```

`RedForge models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise RedForge falls back to matching OAuth/API-key
...
the command uses `RedForge_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, RedForge resolves the input as an alias first, then
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `RedForge_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but RedForge could not resolve a probeable
...
```bash
RedForge models auth add
RedForge models auth login --provider <id>
RedForge models auth setup-token --provider <id>
RedForge models auth paste-token
```
...
`RedForge plugins list` to see which providers are installed.

Examples:

```bash
RedForge models auth login --provider openai-codex --set-default
```
