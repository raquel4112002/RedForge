---
summary: "CLI reference for `RedForge dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: RedForge.internal)
title: "dns"
---
...
```bash
RedForge dns setup
RedForge dns setup --domain RedForge.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `RedForge.internal`)
...
- zone file path
- current tailnet IPs
- recommended `RedForge.json` discovery config
...
- If `--domain` is omitted, RedForge uses `discovery.wideArea.domain` from config.
