---
summary: "CLI reference for `RedForge agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
RedForge agent --to +15555550123 --message "status update" --deliver
...
RedForge agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
RedForge agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: RedForge persists markers from the active source config snapshot, not from resolved runtime secret values.
