---
summary: "CLI reference for `RedForge approvals` and `RedForge exec-policy`"
read_when:
...
## `RedForge exec-policy`

`RedForge exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.RedForge/exec-approvals.json`

Examples:

```bash
RedForge exec-policy show
RedForge exec-policy show --json

RedForge exec-policy preset yolo
RedForge exec-policy preset cautious --json

RedForge exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `RedForge exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `RedForge approvals set --gateway`
or `RedForge approvals set --node <id|name|ip>`.

## Common commands

```bash
RedForge approvals get
RedForge approvals get --node <id|name|ip>
RedForge approvals get --gateway
```

`RedForge approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
RedForge approvals set --file ./exec-approvals.json
RedForge approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
RedForge approvals set --node <id|name|ip> --file ./exec-approvals.json
RedForge approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested RedForge policy aligned, also set:
...
```bash
RedForge approvals allowlist add "~/Projects/**/bin/rg"
RedForge approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
RedForge approvals allowlist add --agent "*" "/usr/bin/uname"

RedForge approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `RedForge nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.RedForge/exec-approvals.json`.
