---
summary: "RedForge CLI reference for `redforge` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.redforge-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.redforge-<name>`.
- `--container <name>`: target a named container for execution.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `redforge update` (source installs only).
...
RedForge uses a lobster palette for CLI output.
...
```
redforge [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `redforge voicecall`).

## Security

- `redforge security audit` — audit config + local state for common security foot-guns.
- `redforge security audit --deep` — best-effort live Gateway probe.
- `redforge security audit --fix` — tighten safe defaults and state/config permissions.
...
- `RedForge plugins list` — discover plugins (use `--json` for machine output).
- `RedForge plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `RedForge plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`; use `--force` to overwrite an existing install target).
- `RedForge plugins marketplace list <marketplace>` — list marketplace entries before install.
- `RedForge plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `RedForge plugins doctor` — report plugin load errors.
...
- `RedForge memory status` — show index stats; use `--deep` for vector + embedding readiness checks or `--fix` to repair stale recall/promotion artifacts.
- `RedForge memory index` — reindex memory files.
- `RedForge memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
- `RedForge memory promote` — rank short-term recalls and optionally append top entries into `MEMORY.md`.
...
- `--install` writes an `RedForge Completion` block into your shell profile and points it at the cached script under the RedForge state directory.
...
- `--workspace <dir>`: agent workspace path (default `~/.RedForge/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `RedForge config` with no
...
- `config set --dry-run`: validate assignments without writing `RedForge.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `RedForge.json`, including propagated field `title` / `description` docs metadata across nested object, wildcard, array-item, and composition branches, plus best-effort live plugin/channel schema metadata.
...
- `RedForge --update` rewrites to `RedForge update`.

### `backup`

Create and verify local backup archives for RedForge state.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs live per-account probe/audit checks when the gateway is reachable; if not, it falls back to config-only channel summaries. Use `RedForge health` or `RedForge status --deep` for broader gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `RedForge doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, RedForge promotes account-scoped values into the channel account map before writing the new account. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```bash
RedForge channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
RedForge channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
RedForge channels remove --channel discord --account work --delete
...
Look up self, peer, and group IDs for channels that expose a directory surface. See [`RedForge directory`](/cli/directory).
...
Tip: use `RedForge skills search`, `RedForge skills install`, and `RedForge skills update` for ClawHub-backed skills.
...
Generate a mobile pairing QR and setup code from the current Gateway config. See [`RedForge qr`](/cli/qr).
...
- After scanning, approve the request with `RedForge devices list` / `RedForge devices approve <requestId>`.

### `clawbot`

Legacy alias namespace. Currently supports `RedForge clawbot qr`, which maps to [`RedForge qr`](/cli/qr).
...
- `hooks enable <name>`
- `hooks disable <name>`
- `hooks install <path-or-spec>` (deprecated alias for `RedForge plugins install`)
- `hooks update [id]` (deprecated alias for `RedForge plugins update`)
...
- Plugin-managed hooks cannot be enabled or disabled through `RedForge hooks`; enable or disable the owning plugin instead.
...
- `setup` configures the Gmail watch plus the RedForge-facing push path.
...
- Without `--apply`, this is a planning helper that prints the recommended RedForge + Tailscale DNS config.
...
- `RedForge message send --target +15555550123 --message "Hi"`
- `RedForge message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, RedForge may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Manage saved MCP server definitions and expose RedForge channels over MCP stdio.

#### `mcp serve`

Expose routed RedForge channel conversations over MCP stdio.
...
- `--gateway`
- `--json`
- node RPC options from `RedForge nodes`
...
RedForge can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `RedForge status --usage` (prints full provider breakdown)
...
- MiniMax note: raw `usage_percent` / `usagePercent` means remaining quota, so RedForge inverts it before display; count-based fields still win when present. `model_remains` responses prefer the chat-model entry, derive the window label from timestamps when needed, and include the model name in the plan label.
- Usage auth comes from provider-specific hooks when available; otherwise RedForge falls back to matching OAuth/API-key credentials from auth profiles, env, or config. If none resolve, usage is hidden.
...
Legacy docs shortcut. Flow commands live under `RedForge tasks flow`:
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named RedForge services are treated as first-class and aren't flagged as "extra".
...
Anthropic note: Anthropic staff told us RedForge-style Claude CLI usage is
allowed again, so RedForge treats Claude CLI reuse and `claude -p` usage as
...
Anthropic setup-token remains available as a supported token-auth path, but RedForge now prefers Claude CLI reuse and `claude -p` when available.

### `models` (root)

`RedForge models` is an alias for `models status`.
...
- Anthropic `setup-token` / `paste-token` remain available as a supported RedForge token path, but RedForge now prefers Claude CLI reuse and `claude -p` when available.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `RedForge_GATEWAY_TOKEN` / `RedForge_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `RedForge_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`RedForge browser`](/cli/browser) and the [Browser tool](/tools/browser).
...
Plugin-provided voice-call utilities. Only appears when the voice-call plugin is installed and enabled. See [`RedForge voicecall`](/cli/voicecall).
...
Search the live RedForge docs index.
