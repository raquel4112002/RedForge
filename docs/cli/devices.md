---
summary: "CLI reference for `RedForge devices` (device pairing + token rotation/revocation)"
read_when:
...
```
RedForge devices remove <deviceId>
RedForge devices remove <deviceId> --json
```

### `RedForge devices clear --yes [--pending]`

Clear paired devices in bulk.

```
RedForge devices clear --yes
RedForge devices clear --yes --pending
RedForge devices clear --yes --pending --json
```

### `RedForge devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, RedForge only prints the selected pending
...
key), RedForge supersedes the previous pending entry and issues a new
`requestId`. Run `RedForge devices list` right before approval to use the
current ID.

```
RedForge devices approve
...
### `RedForge devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
RedForge devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```

Returns the new token payload as JSON.

### `RedForge devices revoke --device <id> --role <role>`
...
```
RedForge devices revoke --device <deviceId> --role node
```
...
```bash
RedForge devices rotate --device <deviceId> --role operator
```
