---
summary: "CLI reference for `RedForge configure` (interactive configuration prompts)"
read_when:
...
Tip: `RedForge config` without a subcommand opens the same wizard. Use
`RedForge config get|set|unset` for non-interactive edits.

For web search, `RedForge configure --section web` lets you choose a provider
...
RedForge configure --section model --section channels
RedForge configure --section gateway --section daemon
```
