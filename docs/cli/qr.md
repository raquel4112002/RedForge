---
summary: "CLI reference for `RedForge qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
RedForge qr
RedForge qr --setup-code-only
RedForge qr --json
RedForge qr --remote
RedForge qr --url wss://gateway.example/ws
```
...
- With `--remote`, RedForge requires either `gateway.remote.url` or
