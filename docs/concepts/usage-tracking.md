...
- `/usage cost` in chats: local cost summary aggregated from RedForge session logs.
- CLI: `RedForge status --usage` prints a full per-provider breakdown.
- CLI: `RedForge channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. RedForge treats
  ...
  quota, so RedForge inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, RedForge prefers the
...
can supply plugin-specific usage auth logic; otherwise RedForge falls back to
