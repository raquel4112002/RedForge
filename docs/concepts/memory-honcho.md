...
[Honcho](https://honcho.dev) adds AI-native memory to RedForge. It persists
...
```bash
RedForge plugins install @honcho-ai/RedForge-honcho
RedForge honcho setup
...
Settings live under `plugins.entries["RedForge-honcho"].config`:
    ...
    entries: {
      "RedForge-honcho": {
        config: {
          ...
          workspaceId: "RedForge", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `RedForge honcho setup` detects and
...
```bash
RedForge honcho setup                        # Configure API key and migrate files
RedForge honcho status                       # Check connection status
RedForge honcho ask <question>               # Query Honcho about the user
RedForge honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/RedForge-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho RedForge integration guide](https://docs.honcho.dev/v3/guides/integrations/RedForge)
- [Memory](/concepts/memory) -- RedForge memory overview
