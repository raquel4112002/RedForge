...
alongside RedForge. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, RedForge falls back to the
...
RedForge creates a self-contained QMD home under
`~/.RedForge/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- RedForge creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, RedForge retries with `qmd query`.
- If QMD fails entirely, RedForge falls back to the builtin SQLite engine.
...
process, so you can tune QMD globally without adding new RedForge config:
...
collection under `~/.RedForge/agents/<id>/qmd/sessions/`.
...
When scope denies a search, RedForge logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If RedForge
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs RedForge uses.
...
RedForge's builtin symlink rules. Keep temporary monorepo checkouts under
