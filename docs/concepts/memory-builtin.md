...
RedForge indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.RedForge/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `RedForge memory index --force`
...
**Memory search disabled?** Check `RedForge memory status`. If no provider is
detected, set one explicitly or add an API key.

**Stale results?** Run `RedForge memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** RedForge falls back to in-process cosine similarity
