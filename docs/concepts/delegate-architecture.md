---
summary: "Delegate architecture: running RedForge as a named agent on behalf of an organization"
...
Goal: run RedForge as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an RedForge agent that:
...
RedForge's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of RedForge's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.RedForge/workspace-delegate",
  tools: {
...
- Cron run history: `~/.RedForge/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.RedForge/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through RedForge's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
RedForge agents add delegate
```

This creates:

- Workspace: `~/.RedForge/workspace-delegate`
- State: `~/.RedForge/agents/delegate/agent`
- Sessions: `~/.RedForge/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.RedForge/workspace" },
      {
        id: "delegate",
        workspace: "~/.RedForge/workspace-delegate",
        tools: {
...
~/.RedForge/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.RedForge/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.RedForge/workspace-org",
        agentDir: "~/.RedForge/agents/org-assistant/agent",
...
recall view. RedForge redacts credential/token-like text, truncates long
