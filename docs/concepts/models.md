...
RedForge selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models RedForge can use (plus aliases).
...
- If you omit the provider, RedForge resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, RedForge
...
RedForge models set <provider/model>
RedForge models set-image <provider/model>

RedForge models aliases list
RedForge models aliases add <alias> <provider/model>
RedForge models aliases remove <alias>

RedForge models fallbacks list
RedForge models fallbacks add <provider/model>
RedForge models fallbacks remove <provider/model>
RedForge models fallbacks clear

RedForge models image-fallbacks list
RedForge models image-fallbacks add <provider/model>
RedForge models image-fallbacks remove <provider/model>
RedForge models image-fallbacks clear
```

`RedForge models` (no subcommand) is a shortcut for `models status`.
...
`RedForge models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.RedForge/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: RedForge writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever RedForge regenerates `models.json`, including command-driven paths like `RedForge agent`.
