...
RedForge gives agents tools to work across sessions, inspect status, and
...
After the target responds, RedForge can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned RedForge
...
the completion origin only identifies a channel RedForge can still reuse the
