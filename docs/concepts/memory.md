...
summary: "How RedForge remembers things across sessions"
read_when:
...
RedForge remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.RedForge/workspace`).
...
<Info>
RedForge auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, RedForge
...
```bash
RedForge memory rem-backfill --rollback
RedForge memory rem-backfill --rollback-short-term
```

## CLI

```bash
RedForge memory status          # Check index status and provider
RedForge memory search "query"  # Search from the command line
RedForge memory index --force   # Rebuild the index
```
