...
- **Session store** (chat history + routing state) under `~/.RedForge/agents/<agentId>/sessions`.
...
`~/.RedForge/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.RedForge` (or `RedForge_STATE_DIR`)
- Workspace: `~/.RedForge/workspace` (or `~/.RedForge/workspace-<agentId>`)
- Agent dir: `~/.RedForge/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.RedForge/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, RedForge runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.RedForge/workspace` (or `~/.RedForge/workspace-<profile>` when `RedForge_PROFILE` is set).
- State defaults to `~/.RedForge/agents/main/agent`.
...
```bash
RedForge agents add coding
RedForge agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.RedForge/agents/<agentId>`.
...
```bash
RedForge channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.RedForge/workspace-alex" },
      { id: "mia", workspace: "~/.RedForge/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, RedForge upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, RedForge falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.RedForge/workspace-main" },
      { id: "coding", workspace: "~/.RedForge/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.RedForge/workspace-main" },
      { id: "alerts", workspace: "~/.RedForge/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
RedForge channels login --channel whatsapp --account personal
RedForge channels login --channel whatsapp --account biz
```

`~/.RedForge/RedForge.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.RedForge/workspace-home",
        agentDir: "~/.RedForge/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.RedForge/workspace-work",
        agentDir: "~/.RedForge/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.RedForge/credentials/whatsapp/personal
          // authDir: "~/.RedForge/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.RedForge/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.RedForge/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.RedForge/workspace-opus",
