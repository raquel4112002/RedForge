---
summary: "What the RedForge system prompt contains and how it is assembled"
read_when:
...
RedForge builds a custom system prompt for every agent run. The prompt is **RedForge-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by RedForge and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full RedForge-owned prompt. The provider runtime can:
...
- **RedForge Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to RedForge docs (repo or npm package) and when to read them.
...
RedForge can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **RedForge
...
occurs, RedForge can inject a warning block in Project Context; control this with
...
When eligible skills exist, RedForge injects a compact **available skills list**
...
local RedForge docs directory (either `docs/` in the repo workspace or the bundled npm
...
for RedForge behavior, commands, configuration, or architecture, and to run
`RedForge status` itself when possible (asking the user only when it lacks access).
