...
RedForge supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, RedForge also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `RedForge configure --section web`. It stores the key in
`~/.RedForge/RedForge.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.RedForge/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
