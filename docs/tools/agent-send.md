...
`RedForge agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    RedForge agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    RedForge agent --agent ops --message "Summarize logs"

    # Target a phone number (derives session key)
    RedForge agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    RedForge agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    RedForge agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    RedForge agent --agent ops --message "Generate report" \
...
RedForge agent --to +15555550123 --message "Trace logs" --verbose on --json

# Turn with thinking level
RedForge agent --session-id 1234 --message "Summarize inbox" --thinking medium

# Deliver to a different channel than the session
RedForge agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
