---
summary: "ClawHub guide: public registry, native RedForge install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **RedForge skills and plugins**.

- Use native `RedForge` commands to search/install/update skills and install
...
```bash
RedForge plugins install clawhub:<package>
...
Native `RedForge` commands install into your active workspace and persist source
...
`RedForge plugins install clawhub:...` only accepts installable plugin families.
If a ClawHub package is actually a skill, RedForge stops and points you at
`RedForge skills install <slug>` instead.

## What ClawHub is

- A public registry for RedForge skills and plugins.
...
4. Users browse, download, and install skills in RedForge.
...
If you want to add new capabilities to your RedForge agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `RedForge skills install <skill-slug>`
3. Start a new RedForge session so it picks up the new skill.
...
## How it fits into RedForge

Native `RedForge skills install` installs into the active workspace `skills/`
directory. `RedForge plugins install clawhub:...` records a normal managed
...
Community or other non-official channels can still install, but RedForge warns
...
current working directory. If an RedForge workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). RedForge loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.RedForge/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches RedForge how to perform a
...
Interested in becoming a moderator? Ask in the RedForge Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to RedForge workspace).
...
Code plugins must include the required RedForge metadata in `package.json`:

```json
{
  "name": "@myorg/RedForge-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "RedForge": {
    "extensions": ["./index.ts"],
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/RedForge/skills` and `~/.RedForge/skills`). This is designed to find older skill installs without extra flags.
