...
`~/.RedForge/RedForge.json`. Agent-specific skill visibility lives under
...
- Built-in skill roots always include `~/.RedForge/skills`, `~/.agents/skills`,
...
- `install.nodeManager`: node installer preference (`npm` | `pnpm` | `yarn` | `bun`, default: npm).
  ...
  - `RedForge setup --node-manager` is narrower and currently accepts `npm`,
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.RedForge.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.RedForge/skills` → bundled skills →
