...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `RedForge sandbox explain`.
...
Legacy `agent.*` configs are migrated by `RedForge doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=RedForge-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${RedForge_STATE_DIR:-$HOME/.RedForge}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
