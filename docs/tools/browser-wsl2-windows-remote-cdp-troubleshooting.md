...
read_when:
  - Running RedForge Gateway in WSL2 while Chrome lives on Windows
...
- RedForge Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/RedForge/RedForge/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- RedForge and Chrome are on the same machine
...
- RedForge points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, RedForge is not the problem yet.
...
Fix that before touching RedForge config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point RedForge at the address that is reachable from WSL2:
...
- use HTTP(S) when you want RedForge to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting RedForge to succeed
...
```bash
RedForge browser open https://example.com --browser-profile remote
RedForge browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `RedForge browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `RedForge browser stop --browser-profile remote`
...
3. RedForge config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug RedForge config or Control UI auth
