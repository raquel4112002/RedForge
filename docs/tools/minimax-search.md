...
RedForge supports MiniMax as a `web_search` provider through the MiniMax
...
RedForge also accepts `MINIMAX_CODING_API_KEY` as an env alias. `MINIMAX_API_KEY`
...
If `plugins.entries.minimax.config.webSearch.region` is unset, RedForge resolves
...
- `query`
- `count` (RedForge trims the returned result list to the requested count)
