...
RedForge can convert outbound replies into audio using ElevenLabs, Microsoft, MiniMax, or OpenAI.
It works anywhere RedForge can send audio.
...
When `messages.tts.provider` is unset, RedForge picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `RedForge.json`.
...
- If `provider` is **unset**, RedForge uses the first configured speech provider in registry auto-select order.
...
`~/.RedForge/settings/tts.json`, override with `RedForge_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, RedForge retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, RedForge:
...
Discord note: `/tts` is a built-in Discord command, so RedForge registers
