...
RedForge supports Grok as a `web_search` provider, using xAI web-grounded
...
`plugins.entries.xai.config.webSearch.apiKey`, RedForge now reuses it as a
...
RedForge can show a separate follow-up step to enable `x_search` with the same
