...
RedForge also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "RedForge plugin SDK" });
    ```
...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of RedForge's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, RedForge keeps the normal managed `web_search` behavior.
...
If no `provider` is set, RedForge checks providers in this order and uses the
...
<Note>
  ...
  RedForge resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
- or omit that field and let RedForge auto-detect the first ready web-fetch
...
Legacy `tools.web.x_search.*` config is auto-migrated by `RedForge doctor --fix`.
When you choose Grok during `RedForge onboard` or `RedForge configure --section web`,
RedForge can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, RedForge does not
...
<Tabs>
  <Tab title="Config file">
    Run `RedForge configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.RedForge/.env`.
...
optional structured filters. RedForge only enables the built-in xAI `x_search`
