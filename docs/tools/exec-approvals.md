...
`ask: "always"` in `~/.RedForge/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `RedForge approvals get`, `RedForge approvals get --gateway`, or
`RedForge approvals get --node <id|name|ip>` to inspect the requested policy,
host policy sources, and the effective result.
For the local machine, `RedForge exec-policy show` exposes the same merged view and
`RedForge exec-policy set|preset` can synchronize the local requested policy with the
local host approvals file in one step. When a local scope requests `host=node`,
`RedForge exec-policy show` reports that scope as node-managed at runtime instead of
...
- **gateway host** → `RedForge` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, RedForge also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.RedForge/exec-approvals.sock",
...
- requested exec policy in RedForge config (`tools.exec.*`)
- host-local approvals policy in `~/.RedForge/exec-approvals.json`
...
- In YOLO mode, RedForge does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
RedForge config set tools.exec.host gateway
RedForge config set tools.exec.security full
RedForge config set tools.exec.ask off
...
```bash
RedForge approvals set --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `RedForge exec-policy` does not synchronize node approvals
- `RedForge exec-policy set --host node` is rejected
- node exec approvals are fetched from the node at runtime, so node-targeted updates must use `RedForge approvals --node ...`
...
When `tools.exec.strictInlineEval=true`, RedForge treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.RedForge/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `RedForge approvals allowlist ...`).
- `RedForge security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `RedForge doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, RedForge still rejects the `env` builtin in safe-bin
...
`~/.RedForge/exec-approvals.json` directly.

CLI: `RedForge approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, RedForge sends a followup `agent` turn to the same session.
...
topics, RedForge preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, RedForge prevents the agent from reusing
