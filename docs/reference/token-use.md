---
summary: "How RedForge builds prompt context and reports token usage + costs"
read_when:
...
RedForge tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

RedForge assembles its own system prompt on every run. It includes:
...
For images, RedForge downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from RedForge session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `RedForge status --usage` and `RedForge channels list` show
...
available; otherwise RedForge falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, RedForge shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. RedForge can
...
Anthropic's 1M context window is currently beta-gated. RedForge can inject the
...
RedForge skips the `context-1m-*` beta header because Anthropic currently
