...
This document explains how RedForge manages sessions end-to-end:
...
RedForge is designed around a single **Gateway process** that owns session state.
...
RedForge persists sessions in two layers:
...
- Transcripts: `~/.RedForge/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

RedForge resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, RedForge reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
RedForge sessions cleanup --dry-run
RedForge sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.RedForge/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
RedForge intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When RedForge splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, RedForge
...
- If a trailing tool-result block would otherwise push the chunk over target,
  RedForge preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (RedForge consumes the events, but Pi decides when to compact).
...
RedForge also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, RedForge bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, RedForge leaves it alone.
...
- If the provider fails or returns an empty result, RedForge falls back to built-in LLM summarization automatically.
...
- `RedForge status` (CLI)
- `RedForge sessions` / `sessions --json`
...
RedForge supports “silent” turns for background tasks where the user should not see intermediate output.
...
- RedForge strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, RedForge also suppresses **draft/typing streaming** when a
...
RedForge uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but RedForge’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `RedForge status`.
