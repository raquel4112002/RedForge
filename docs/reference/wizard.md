...
This is the full reference for `RedForge onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.RedForge/RedForge.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `RedForge doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though RedForge now prefers Claude CLI reuse when available.
    - **OpenAI Code (Codex) subscription (Codex CLI)**: if `~/.codex/auth.json` exists, onboarding can reuse it. Reused Codex CLI credentials stay managed by Codex CLI; on expiry RedForge re-reads that source first and, when the provider can refresh it, writes the refreshed credential back to Codex storage instead of taking ownership itself.
    ...
    - Auth profiles live in `~/.RedForge/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.RedForge/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.RedForge/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$RedForge_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.RedForge/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `RedForge pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `RedForge configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `RedForge health`.
    - Tip: `RedForge status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
  </Step>
...
```bash
export RedForge_GATEWAY_TOKEN="your-token"
RedForge onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env RedForge_GATEWAY_TOKEN
```
...
- Stores it under `~/.RedForge/tools/signal-cli/<version>/`.
...
Typical fields in `~/.RedForge/RedForge.json`:
...
`RedForge agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.RedForge/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.RedForge/agents/<agentId>/sessions/`.
