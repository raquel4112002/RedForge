...
RedForge features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, RedForge reads usage from
...
Anthropic still does not expose a per-message dollar estimate that RedForge can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `RedForge status --usage` and `RedForge channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so RedForge inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, RedForge prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise RedForge falls back to matching OAuth/API-key
...
RedForge can pick up credentials from:
...
RedForge's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's RedForge Claude-login path with **Extra Usage** enabled.
...
- `RedForge status --usage`
- `RedForge models status --json`
...
`RedForge models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
