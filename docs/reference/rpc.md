...
RedForge integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- RedForge owns lifecycle when `channels.signal.autoStart=true`.
...
- RedForge spawns `imsg rpc` as a child process (legacy iMessage integration).
