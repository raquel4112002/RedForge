...
function validateClawHubPluginPackage(params: {
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this ${PRODUCT_DISPLAY_NAME} runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires ${PRODUCT_DISPLAY_NAME} >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
