import { slugifyRedForgeId } from "../../commands/redforge.shared.js";
import {
  detectTargetValue,
  gateMissionIntent,
  inferGateWarnings,
...
export function buildScopeDraft(params: {
  ...
  return {
    id: slugifyRedForgeId(description),
    description,
...
export function buildMissionDraft(params: {
  ...
  return {
    id: slugifyRedForgeId(objective),
    objective,
...
export function buildMissionPlan(prompt: string, workspaceDir: string): MissionPlan {
  ...
  const targetId = slugifyRedForgeId(targetName);
