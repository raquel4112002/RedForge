...
const SEARCH_TOOL = "https://docs.redforge.ai/mcp.SearchRedForge";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.redforge.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('redforge docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.redforge.ai/");
      runtime.log(`Search: ${formatCliCommand('redforge docs "your query"')}`);
    }
    return;
  }
