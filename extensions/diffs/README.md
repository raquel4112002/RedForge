...
Read-only diff viewer plugin for **RedForge** agents.
...
Set plugin-wide defaults in `~/.RedForge/RedForge.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/RedForge",
        },
      },
    },
  },
...
Before:
RedForge supports plugins.

After:
RedForge supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/RedForge-diffs`).
