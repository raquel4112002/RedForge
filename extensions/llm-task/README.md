...
`RedForge.invoke --each`) without adding new RedForge code per workflow.
...
This extension depends on RedForge internal modules (the embedded agent runner).
It is intended to ship as a **bundled** RedForge extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.RedForge/extensions` as a standalone plugin directory.
