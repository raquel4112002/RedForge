# @RedForge/memory-wiki

Persistent wiki compiler and Obsidian-friendly knowledge vault for **RedForge**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.RedForge-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
RedForge wiki get entity.alpha --from 1 --lines 80

RedForge wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

RedForge wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

RedForge wiki bridge import
RedForge wiki unsafe-local import

RedForge wiki obsidian status
RedForge wiki obsidian search "alpha"
RedForge wiki obsidian open syntheses/alpha-summary.md
RedForge wiki obsidian command workspace:quick-switcher
RedForge wiki obsidian daily
```
...
- The compiled agent digests in `.RedForge-wiki/cache/agent-digest.json` and `.RedForge-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
