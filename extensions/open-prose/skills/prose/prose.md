...
- **Task tool** in the upstream spec == RedForge `sessions_spawn`
- **File I/O** == RedForge `read`/`write`
- **Remote fetch** == RedForge `web_fetch` (or `exec` with curl when POST is required)
