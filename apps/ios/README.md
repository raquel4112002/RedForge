# RedForge iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to a RedForge Gateway as a `role: node`.
...
- Beta release uses canonical `ai.RedForge.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `RedForgePushTransport=relay`, `RedForgePushDistribution=official`, and `RedForgePushAPNsEnvironment=production`.
...
- `RedForge_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export RedForge_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/RedForge-<version>.ipa`
   - `apps/ios/build/beta/RedForge-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/RedForge.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `RedForgePushTransport=direct` and `RedForgePushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `RedForge_APNS_TEAM_ID`, `RedForge_APNS_KEY_ID`, and either `RedForge_APNS_PRIVATE_KEY_P8` or `RedForge_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.RedForge/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `RedForge_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `RedForgePushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `RedForge.json`. `RedForge_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine RedForge official builds and to ensure a
